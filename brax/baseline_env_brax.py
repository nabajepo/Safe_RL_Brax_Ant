...
class Cfg:
    ...
    # -----------------------------------------------------
    start_clearance_extra: float = 0.50
    goal_clearance_extra: float = 0.60
...
class BraxAntBase:
    ...
    def _sample_goal_for_one_env(
        ...
        # fallback
        return (start + np.array([3.0, 0.0], dtype=np.float32)).astype(np.float32)
    ...
    def _is_obstacle_valid(
        ...
        if d_start <= (r + self.cfg.agent_r + self.cfg.start_clearance_extra):
            return False

        # keep away from goal
        d_goal = float(np.linalg.norm(c - goal))
        if d_goal <= (r + self.cfg.agent_r + self.cfg.goal_clearance_extra):
            return False
    ...
    def _sample_path_obstacle_for_one_env(
        ...
        # fallback
        c = ((start + goal) * 0.5).astype(np.float32)
        return c, 0.45
    ...
    def _sample_obstacles_for_one_env(
        ...
        return (
            np.stack(obs_xy, axis=0).astype(np.float32),
            np.array(obs_r, dtype=np.float32),
        )
