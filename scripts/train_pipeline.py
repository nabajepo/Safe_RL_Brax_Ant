...
import envs.no_constraint_env as no_constraint_env  # noqa: F401
import envs.soft_constraint_env as soft_constraint_env  # noqa: F401
import envs.hard_constraint_env as hard_constraint_env  # noqa: F401

from envs.baseline_env import Cfg, get_env_class
...
# =========================================================
def load_checkpoint_for_resume(model_dir, args, cfg):
    """
    Load the latest checkpoint if it exists.
    Returns: (params, opt_state, start_iter, start_timesteps, curve_records)
    """
    ...
    if checkpoint_dir.exists():
        for ckpt_file in checkpoint_dir.glob("checkpoint_*.pkl"):
            # Extract timesteps from filename like checkpoint_t10k.pkl or checkpoint_t1m.pkl
            name = ckpt_file.stem  # "checkpoint_t10k"
            if "checkpoint_" in name:
                tag = name.replace("checkpoint_", "")
                if tag.endswith("k"):
                    timesteps = int(tag[:-1]) * 1000
                elif tag.endswith("m"):
                    timesteps = int(tag[:-1]) * 1_000_000
                else:
                    continue
    ...
    # Also check latest_model.pkl (more recent than periodic checkpoints)
    latest_model_path = model_dir / "latest_model.pkl"
    if latest_model_path.exists():
        # Get modification time or timesteps from metadata
        try:
            payload = load_training_state(latest_model_path)
            meta = payload.get("meta", {})
            ckpt_timesteps = meta.get("timesteps", 0)
            if ckpt_timesteps > latest_timesteps:
                latest_timesteps = ckpt_timesteps
    ...
    # Load the checkpoint
    payload = load_training_state(latest_checkpoint)
    params = payload["params"]
    opt_state = payload["opt_state"]
    meta = payload.get("meta", {})
    
    start_iter = meta.get("iteration", 0)
    start_timesteps = meta.get("timesteps", 0)
    
    # Load previous learning curve if it exists
    curve_csv_path = model_dir / "curves" / "learning_curve.csv"
    curve_records = []
    if curve_csv_path.exists():
        import pandas as pd
        df = pd.read_csv(curve_csv_path)
        curve_records = df.to_dict(orient="records")
    
    print(f"Resuming from checkpoint: {latest_checkpoint}")
    print(f"  - Resuming at iteration: {start_iter}")
    print(f"  - Resuming at timesteps: {start_timesteps}")
    
    return params, opt_state, start_iter, start_timesteps, curve_records

# =========================================================
# Training per seed
# =========================================================
def run_single_seed(model_name, seed, args, model_budget_dir, cfg, auto_resume=True):
    ...
    params, opt_state, start_iter, start_timesteps, curve_records = load_checkpoint_for_resume(
        model_dir=model_dir,
        args=args,
        cfg=cfg,
    )
    ...
    if params is None:
        ...
        start_timesteps = 0
        curve_records = []
        print(f"[{model_name}] Fresh start for seed {seed}")
    else:
        # Recreate optimizer (must match the saved state's optimizer type)
        optimizer = optax.chain(
            optax.clip_by_global_norm(args.max_grad_norm),
            optax.adam(args.learning_rate),
        )
        # opt_state is already loaded from checkpoint
        print(f"[{model_name}] Resumed seed {seed} from iteration {start_iter}")
    ...
    for it in range(start_iter + 1, num_iters + 1):
        ...
        current_timesteps = start_timesteps + (it - start_iter) * steps_per_iter
    ...
    # Reload best model params for rollouts
    best_model_path = model_dir / "best_model.pkl"
    if best_model_path.exists():
        best_payload = load_training_state(best_model_path)
        rollout_params = best_payload["params"]
        print(f"[{model_name}] Seed {seed} — using best model for rollouts", flush=True)
    else:
        rollout_params = params
        print(f"[{model_name}] Seed {seed} — best_model.pkl not found, using latest params for rollouts", flush=True)
        rollout_rows = []
    ...
    for k in range(args.rollouts_per_seed):
        ...
        pickle_payload = {
            "traj_xy":    rollout_data["traj_xy"],
            "goal_xy":    rollout_data["goal_xy"],
            "obs_xy":     rollout_data["obs_xy"],
            "obs_r":      rollout_data["obs_r"],
            "metrics":    rollout_data["metrics"],
            "cfg":        cfg.__dict__,          # arena_size, goal_radius, agent_r, buffer_dist …
            "model_name": model_name,
            "seed":       rollout_seed,
        }
        save_pickle(pickle_payload, rollouts_dir / f"rollout_{k}_data.pkl")
...
def plot_per_metric_across_seeds(model_name, model_budget_dir, seeds):
    """
        For each metric, produce ONE plot that shows:
        - a mean line (aggregated across seeds)
        - a ±1 std shaded band
        - individual seed lines as thin background traces
        Saved to: learning_curves/<metric>.png
    """
    learning_curves_dir = model_budget_dir / "learning_curves"
    ensure_dir(learning_curves_dir)
    ...
    # Load all seed dataframes
    seed_frames = {}
    for seed in seeds:
        csv_path = model_budget_dir / f"seed_{seed}" / "curves" / "learning_curve.csv"
        if csv_path.exists():
            seed_frames[seed] = pd.read_csv(csv_path)
    ...
    for metric in metrics:
        ...
        # Collect per-seed series aligned on iteration
        all_x = []
        all_y = []
        ...
        for seed, df in seed_frames.items():
            ...
            ax.plot(x_arr, y_arr, linewidth=0.9, alpha=0.35, color="steelblue")
            all_x.append(x_arr)
            all_y.append(y_arr)
            plotted_any = True
        ...
        # Aggregate: align on shortest common length
        min_len = min(len(a) for a in all_x)
        x_common = all_x[0][:min_len]
        y_mat = np.stack([a[:min_len] for a in all_y], axis=0)  # (n_seeds, T)

        mean_y = y_mat.mean(axis=0)
        std_y  = y_mat.std(axis=0)

        ax.plot(x_common, mean_y, linewidth=2.2, color="steelblue", label="mean")
        ax.fill_between(
            x_common,
            mean_y - std_y,
            mean_y + std_y,
        ...
        ax.set_title(f"{metric} — {model_name}  (mean ± std, n={len(seed_frames)} seeds)")
